...
#ifndef SHAPES_INTERSECTION_H
#define SHAPES_INTERSECTION_H 1
...
template <typename T>
class Intersection : public Structure<T>
{
	public:
		typedef typename Structure<T>::FPPoint  FPPoint;
		typedef typename Structure<T>::FPVector FPVector;

		Intersection(const std::string name__ = "", T _exponent = 2.) :
			Structure<T>(name__), exponent(_exponent) { }

		Intersection(T _exponent, const std::string name__ = "") :
			Structure<T>(name__), exponent(_exponent) { }
...
#include <shapes/Intersection.hh>
