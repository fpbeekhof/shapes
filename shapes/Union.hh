...
template <typename T>
void Union<T>::add(Structure<T> *structure)
{
	structures.push_back(structure);
}

template <typename T>
T Union<T>::rawValue(const FPPoint &p) const
{
	T val = 0.0;
	...
	return std::pow(val, (1.0f / exponent) );
}

template <typename T>
bool Union<T>::fromXML(TiXmlHandle &root)
{
...
template <typename T>
TiXmlElement * const Union<T>::toXML() const
{
	using boost::lexical_cast;

	TiXmlElement * const root = new TiXmlElement("Union");
...
template <typename T>
void Union<T>::print(unsigned indent) const
{
	using boost::lexical_cast;

	Structure<T>::printIndented("<Union>", indent);
	std::string buf;
/*
	if (this->exponent != 2)
	{
...
*/
	...
	Structure<T>::printIndented("</Union>\n", indent);
}
